...
#define OUTPUT_FILE "output/openmp.bmp"
#define BILLION 1E9
FILE *fp;
...
// Median filtering
void _medianfilter_chunck( unsigned char** image,unsigned char** result,int nCols,int nRows,int strips)
{
	#pragma omp parallel
	{
		unsigned char *local;
...
int main(int argc,char *argv[])
{
  ...
  getBMP(bmpInput,tmp,rasterOffset,chunck);
	...
	_medianfilter_chunck( pixel, result, nCols, nRows/nr_threads,strips);
	...
	//for(int k=0;k<nr_threads-1;k++)
	//	_medianfilter_correction(pixel[k+1],pixel[k],result[k+1],result[k],nCols,nRows/nr_threads);

...
int main(int argc,char *argv[])
{
	...
	unsigned char **pixel=(	unsigned char **) malloc(4 * sizeof(char*));
	unsigned char **result=( unsigned char **) malloc(4 * sizeof(char*));
	char signature[2];
	...
	printf("\n\nAlocare spatiu: ");
	int chunck=nRows*nCols/4;
	/*pixel0 = (char *) calloc(chunck,sizeof(char));
	pixel1 = (char *) calloc(chunck,sizeof(char));
	pixel2 = (char *) calloc(chunck,sizeof(char));
	pixel3 = (char *) calloc(chunck,sizeof(char));
	
	result0 = (char *) malloc(chunck*sizeof(char));
	result1 = (char *) malloc(chunck*sizeof(char));
	result2 = (char *) malloc(chunck*sizeof(char));
	result3 = (char *) malloc(chunck*sizeof(char));
	...
	for(int i=0;i<4;i++)
	{
		pixel[i]=(char *) calloc(chunck,sizeof(char));
		result[i]=(char *) malloc(chunck*sizeof(char));
		
	}
	
	 pthread_t some_thread;
    struct image_chunck* images=(struct image_chunck*) malloc(4*sizeof(struct image_chunck));
	for(int i=0;i<4;i++)
	{
		images[i].nCols=nCols/2;
	    images[i].nRows=nRows/2;
	...
	for(int k=0;k<4;k++)
	{
		...
		for(int i=0;i < chunck;i++)
		{
			...
			tmp[i] = (char)getImageInfo(bmpInput, rasterOffset + i + chunck*k, 1);
    ...
    t = clock();
	 ...
	 //create all threads one by one
	  for (i = 0; i < 4; i++) {
		...
		pthread_create(&threads[i], NULL, _medianfilter_chunck, &images[i]);

...
// Median filtering
void _medianfilter_chunck( unsigned char** image,unsigned char** result,int nCols,int nRows)
{	
	#pragma omp parallel 
	{
		unsigned char *local;	
		unsigned char *local_result;		
		int tid = omp_get_thread_num(); //id-ul de la 0 la nr_thread-1
		printf("\nId :%d \n",tid);
		local = image[tid];
		local_result = result[tid];
...
int main(int argc,char *argv[])
{
	...
	clock_t t; 
    	t = clock();
	_medianfilter_chunck( pixel, result, nCols/2, nRows/2);
	t = clock() - t; 
